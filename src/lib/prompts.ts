...
- BRAND-PREFIX TRAP: never combine a hotel-, restaurant-, or chain-brand name (e.g. "Gino", "Hilton", "Radisson", "Hyatt", "Marriott", "McDonald's", "Starbucks", "Holiday Inn", "Hard Rock", "Rooms") with a generic suffix that sounds attraction-shaped ("Art Center", "Bread Factory", "Cultural Space", "Gallery", "Garden", "Tower", "House") unless that exact combination is a documented landmark with its own Wikipedia article (the Hard Rock Cafe in some cities, the Empire State Building's Tiffany lobby, etc.). The failure mode is taking a real famous place ("Fabrika" in Tbilisi — the former Soviet bread factory turned cultural hub) and prefixing it with an unrelated brand the model has seen nearby ("Gino" hotels also exist in Tbilisi). Brand-prefixed venues are almost always private commercial properties — hotels, restaurants, member clubs — NOT cultural attractions. Exclude on sight.
