# Lokali — AI Prompt-ების სრული რეფერენსი

ერთიანი ცნობარი ყველა AI prompt-ის რომელსაც Lokali იყენებს runtime-ში.
განახლების თარიღი: 2026-06-11.

| # | Prompt | მოდელი | ფაილი | დანიშნულება |
|---|--------|--------|-------|-------------|
| 1 | Attractions | Claude Sonnet 4.x | `src/lib/prompts.ts` → `buildAttractionsSystem` + `buildAttractionsUser` | ქალაქის/ქვეყნის attraction-ების ჩამონათვალი (10–30) |
| 2 | Guide | Claude Sonnet 4.x | `src/lib/prompts.ts` → `buildGuideSystem` + `buildGuideUser` | ერთი attraction-ის audio-გიდი (6–10 წუთი) |
| 3 | Museum highlights | Claude Sonnet 4.x | `src/lib/prompts.ts` → `buildMuseumHighlightsSystem` + `buildMuseumHighlightsUser` | მუზეუმის Top-30 ექსპონატი |
| 4 | Time Machine | Claude Sonnet 4.x | `src/lib/prompts.ts` → `buildTimeMachineSystem` + `buildTimeMachineUser` | ისტორიული მომენტის first-person სიმულაცია |
| 5 | Name normalization | Claude Haiku 4.5 | `src/lib/normalizeAttractionName.server.ts` → `SYSTEM_PROMPT` | non-English attraction name → canonical English (cache key) |
| 6 | Translation | Gemini Flash 2.0 | `src/lib/geminiTranslate.server.ts` → `systemInstruction` | runtime UI/content translation 35 ენაზე |

---

## 1. Attractions

ვიყენებთ `/api/attractions`-ში. შედეგად ვიღებთ JSON object-ს `{ "attractions": [...] }`.

### System Prompt — `buildAttractionsSystem()`

```text
You are a travel curator for Lokali, an AI-powered audio-guide app for explorers. Your job is to return a curated list of real, visitable attractions for the user's travel query.
...
- "category": Choose exactly ONE id from this set: <INTEREST IDS, injected from INTERESTS array>. Pick the strongest fit; default to "editors" when no clear bias.
...
### User Prompt — `buildAttractionsUser(args)`
...
[optional] INTEREST BIAS (optional, just for ranking — do not exclude other types): <interests joined by ", ">
...
[OR if no excludes]
EXCLUDE: (none — return your strongest top picks first)
...
## 2. Guide (Audio Tour for one Attraction)

ვიყენებთ `/api/guide`-ში. შედეგად ვიღებთ object-ს `{ title, script, estimated_duration_seconds, key_facts, tips, look_for }`.

### System Prompt — `buildGuideSystem()`

```text
You are a master audio-guide writer for Lokali. Your job is to produce a cinematic, warm, knowledgeable narrated guide for a single attraction — the kind of guide a great BBC travel documentary would script. The output is read aloud by a TTS voice, so write for the EAR.
...
### User Prompt — `buildGuideUser(args)`

```text
ATTRACTION: <args.name>
[optional] CITY: <args.city>
LANGUAGE: <args.language || "en">
INTEREST: <args.interest || "editors">
...
ვიყენებთ მუზეუმის გვერდზე (`/api/museum-highlights`). შედეგად ვიღებთ Top-30 ექსპონატს.

### System Prompt — `buildMuseumHighlightsSystem()`

```text
You are a senior museum curator writing a first-visit highlights guide for one specific museum. The app shows 10 items per page, up to three pages — so order matters.
...
### User Prompt — `buildMuseumHighlightsUser(args)`
...
### User Prompt — `buildTimeMachineUser(args)`
...
ROLE: <args.role>
ROLE BRIEF: <ROLE_BRIEFS[args.role]>
LANGUAGE: <args.language || "en">
...
### Role briefs (injected into User Prompt as `<ROLE_BRIEFS[args.role]>`)

```text
merchant  → A travelling trader. Notices prices, goods, who's buying. Talks to ship captains and innkeepers. Knows which roads are safe.
soldier   → A common soldier in the local army. Notices defences, ranks, mood in the barracks. Loyal but tired. Carries his weapon everywhere.
servant   → A household servant or labourer. Sees the powerful from below — overhears, fetches, cleans. Invisible to most, present for everything.
foreigner → A stranger from a distant land. Notices what locals take for granted. Doesn't speak the language fluently. Compares everything to home.
child     → A child of about 10. Sees the world with wonder and confusion. Fixates on small details adults miss. Doesn't fully understand the danger or the politics.
healer    → A folk healer / physician. Knows herbs, wounds, fevers. People come with bodies and stories. Has seen many deaths.
spy       → An informant working for a foreign power or rival faction. Watches everything, trusts no one, carries a hidden purpose.
survivor  → Someone who lived through what is about to happen and is recounting it years later — the voice carries the weight of memory and loss.
```

---

## 5. Name Normalization (cache-key canonicalization)

ვიყენებთ ყოველ cold-cache attraction request-ში თუ source language არ არის ინგლისური და query შეიცავს non-ASCII სიმბოლოებს. შედეგი = canonical English name, რომელიც cache-კიდ ექცევა. ENG ან pure-ASCII inputs short-circuit-ში გადადიან Haiku-ის გარეშე.

### System Prompt — `SYSTEM_PROMPT` (Haiku 4.5)

```text
You normalize a single place name into its canonical English form.

The input may be a landmark, attraction, neighborhood, park, museum, religious site, natural site, city, region, or country. It may be written in Georgian, Cyrillic, Arabic, Persian, Chinese, Japanese, Korean, Thai, Devanagari, English, or phonetic transliteration.
...
```text
<the raw input place name — single line, e.g. "პაკ ხლონგ ტალატი">
```

ტექნიკური პარამეტრები: `model: "claude-haiku-4-5"`, `maxTokens: 64`, `temperature: 0.1`.

---

## 6. Translation (runtime UI + content)

ვიყენებთ `/api/translate`-ში. ეტაპები:
1. **Gemini Flash 2.0** — primary (კონტექსტ-aware, ~$2 / 1M chars).
2. **Google Cloud Translation v2 Basic** — fallback თუ Gemini-მ source-ი უცვლელად დააბრუნა.
3. **Source unchanged** — საბოლოო fallback თუ ორივე ჩავარდა.

### System Instruction — Gemini Flash 2.0 (`geminiTranslateBatch`)

```text
You are a translation engine. Translate every input string into <TARGET LANGUAGE NAME>.
...
Strict rules:
  ...
  - Preserve proper nouns (museum names, artist names, brand names) in their original form unless <TARGET LANGUAGE NAME> has a well-established conventional spelling.
  - For country and city names, use the canonical <TARGET LANGUAGE NAME> form (e.g. "Georgia" → the South Caucasus country, not the US state, when context is about a city or museum).
  - Never return the source text untranslated unless it is a brand name, code, or already in <TARGET LANGUAGE NAME>.
```
...
## 7. TTS (Azure Speech) — არ არის "AI prompt"

`/api/tts` მხოლოდ SSML envelope-ს ხატავს Azure Speech REST API-სთვის — ეს არ არის LLM prompt, არამედ XML wrapper. სტრუქტურა:
...
ვისაც აქ რაიმე "prompt engineering" ეხება — სცენარის ტექსტი მზადდება ეტაპ 2-ში (Guide) ან ეტაპ 4-ში (Time Machine). TTS-ი იღებს უკვე მზა ტექსტს.

---

## სად შევცვალოთ ეს prompt-ები

| Prompt | ფაილი | ფუნქცია |
|--------|-------|---------|
| Attractions | `src/lib/prompts.ts` | `buildAttractionsSystem` / `buildAttractionsUser` |
| Guide | `src/lib/prompts.ts` | `buildGuideSystem` / `buildGuideUser` |
| Museum Highlights | `src/lib/prompts.ts` | `buildMuseumHighlightsSystem` / `buildMuseumHighlightsUser` |
| Time Machine | `src/lib/prompts.ts` | `buildTimeMachineSystem` / `buildTimeMachineUser` (+ `ROLE_BRIEFS`) |
| Name Normalization | `src/lib/normalizeAttractionName.server.ts` | `SYSTEM_PROMPT` |
| Translation | `src/lib/geminiTranslate.server.ts` | `systemInstruction` (inline string) |

ცვლილების მერე → commit → Lovable auto-deploy → ახალი cold requests ახალ prompt-ს იყენებენ. `cached_guides` / `cached_attractions` / `cached_museum_highlights` ცხრილებში უკვე შენახული შედეგები ძველი prompt-ის შედეგებია — თუ გვინდა გადახედვა, საჭიროა შესაბამისი ცხრილების ჩასუფთავება.
